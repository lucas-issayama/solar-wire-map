...
import { fieldsShippingAddress } from "@/types/fields/fields-shipping-address";
import { formatAddressSingleLine } from "@/utils/format/format-address-single-line";

export default function QuoteCardSelectShippingAddress({
  fieldsFromParent,
  handleClickItem,
  quote,
  setQuote,
    ...
    <div className="">
      {quote?.billingEntity == "enterprise" && (
        <FieldInputData
          ...
          customFields={fieldsShippingAddress}
      ...
      {/* {addressFieldName && quote && <div>{quote[addressFieldName]?.name}</div>} */}
    </div>
  );
}
