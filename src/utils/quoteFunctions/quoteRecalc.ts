...
import { getItemPrice } from "../sap/get-item-price";
import { getItemPriceRecalc } from "../sap/get-item-price-recalc";
import { getItemPriceFinal } from "../sap/get-item-price-final";
import { getQuoteDcAcPowerFactor } from "../format/get-quote-dc-ac-power-factor";
import { getQuoteAcPowerInverter } from "../format/get-quote-ac-power-inverters";
...
let machineInterestRates = [
  ...
  //2025-08-01
  { min: 1, max: 1, interestRate: 2.35 },
  { min: 2, max: 6, interestRate: 2.65 },
  { min: 7, max: 12, interestRate: 2.95 },
  { min: 13, max: 18, interestRate: 3.45 },
];

// let taxesFactorSingleItems = 1 - 0.0925 - 0.0975 - 0.18;
// let taxesFactorKit = 1 - 0.0925;

function updatePricesItemsAndTaxes(quote: Quote) {
  ...
  for (let i = 0; i < length; i++) {
    if (quote?.quoteKits?.[0]?.quoteKitItems?.[i]) {
      if (
        ...
        let itemPriceRaw = getItemPrice(quote, quoteKitItem);
        let itemPrice;
        if (quoteKitItem.type == "module") {
          ...
          if (isQuoteSingleItems(quote))
            itemPrice =
              (taxesFactorKit / taxesFactorSingleItems) * itemPriceRaw;
          else itemPrice = itemPriceRaw;
        }
        quote.quoteKits[0].quoteKitItems[i].finalPriceInCents =
          Math.round(itemPrice);
        quote.quoteKits[0].quoteKitItems[i].finalRevenueInCents = Math.round(
          itemPriceRaw * taxesFactorKit
        );
        total += itemPrice * quoteKitItem.quantity;
      } else {
  ...
  if (quote?.quoteKits?.[0]?.quoteKitItems) {
    console.log("revenue");
    console.log(
      quote.quoteKits[0].quoteKitItems.reduce(
        (acc: any, cur) => acc + cur.quantity * cur.finalRevenueInCents,
        0
      )
    );
  }

  quote.priceInCents = Math.round(total);
  return quote;
}

function updatePricesItemsFinal(quote: Quote) {
  let length = quote?.quoteKits?.[0]?.quoteKitItems?.length ?? 0;
  let total = 0;
  for (let i = 0; i < length; i++) {
    if (quote?.quoteKits?.[0]?.quoteKitItems?.[i]) {
      if (
        ...
        let quoteKitItem: any = quote?.quoteKits?.[0]?.quoteKitItems?.[i];
        let itemPriceRaw = getItemPriceFinal(quote, quoteKitItem);
        let itemPrice;
        let itemRevenue;
        if (quoteKitItem.type == "module") {
...
function getRevenueInCents(quote: Quote) {
  let length = quote?.quoteKits?.[0]?.quoteKitItems?.length ?? 0;
  let total = 0;
  for (let i = 0; i < length; i++) {
    let quoteKitItem: any = quote?.quoteKits?.[0]?.quoteKitItems?.[i];

    total += quoteKitItem.quantity * quoteKitItem.finalRevenueInCents;
  }

  //Debug
  if (quote?.quoteKits?.[0]?.quoteKitItems) {
    console.log("revenue kit final");
    console.log(
      quote.quoteKits[0].quoteKitItems.reduce(
        (acc: any, cur) => acc + cur.quantity * cur.finalRevenueInCents,
        0
    ...
    console.log("price kit final");
    console.log(
      quote.quoteKits[0].quoteKitItems.reduce(
        (acc: any, cur) => acc + cur.quantity * cur.finalPriceInCents,
        0
      )
    );
  }
...
function getTaxesInCents(quote: Quote) {
  let length = quote?.quoteKits?.[0]?.quoteKitItems?.length ?? 0;
  let total = 0;
  for (let i = 0; i < length; i++) {
    let quoteKitItem: any = quote?.quoteKits?.[0]?.quoteKitItems?.[i];
    ...
    total +=
      quoteKitItem.quantity * quoteKitItem.finalPriceInCents -
      quoteKitItem.quantity * quoteKitItem.finalRevenueInCents;
  }

  console.log(JSON.stringify({ taxes: total }));

  return Math.round(total);
}

function getKitTaxesInCents(quote: Quote) {
  let length = quote?.quoteKits?.[0]?.quoteKitItems?.length ?? 0;
  let taxesInCents = 0;
  for (let i = 0; i < length; i++) {
    if (quote?.quoteKits?.[0]?.quoteKitItems?.[i]) {
      if (
        ...
        let quoteKitItem: any = quote?.quoteKits?.[0]?.quoteKitItems?.[i];
        //alert(quote.priceKitsFinalInCents);
        let itemPriceRaw = getItemPriceRecalc(
          quote,
          quoteKitItem,
          quote.priceKitsFinalInCents
        );
        //  alert(JSON.stringify({ itemPriceRaw }));
        let itemPrice;
        let itemRevenue;
        if (quoteKitItem.type == "module") {
          itemRevenue = itemPriceRaw * taxesFactorKit;
        ...
        taxesInCents +=
          (Math.round(itemPriceRaw) - Math.round(itemRevenue)) *
          quoteKitItem.quantity;

        //  alert(JSON.stringify(taxesInCents));
      }
    }
  }

  return Math.round(taxesInCents);
}
...
function quoteRecalc(quote: Quote, deleteApproval?: boolean) {
  ...
  if (quote) {
    quote.quoteKits[0].singleItems =
      getQuoteDcAcPowerFactor(quote) < 0.2 ||
      getQuoteAcPowerInverter(quote) <= 0;
  }
  ...
  if (quoteUpdated.paymentMethod?.id == 2) {
    ...
    quoteUpdated.paymentInterestAmountInCents = Math.round(
      paymentInfo?.totalInterestAmount
    );
    quoteUpdated.priceInCents += quoteUpdated.paymentInterestAmountInCents;

    let interestFactor =
      quoteUpdated.paymentInterestAmountInCents /
      quoteUpdated.priceKitsFinalInCents;
    costsInPercentage.push(interestFactor);
  }

  //Fix taxes
  quoteUpdated = updatePricesItemsAndTaxes(quoteUpdated);

  //Calc final price
  quoteUpdated.rawPriceInCents = quoteUpdated.priceInCents;
  //Recalc

  if (!quoteUpdated.discountFinalValue) quoteUpdated.discountFinalValue = 0;
  ...
  if (doRecalc) {
    ...
    if (quoteUpdated.paymentMethod?.id == 2) {
      ...
      //Avaliar se faz sentido aplicar o imposto aqui para a correta composicao do custo
      quoteUpdated.paymentInterestAmountInCents = Math.round(
        paymentInfo?.totalInterestAmount
      );
    }
    ...
    //Fix taxes -- check how to do it
    quoteUpdated = updatePricesItemsFinal(quoteUpdated);
    //quoteUpdated = updatePricesItemsAndTaxes(quoteUpdated);
  }
  //}//********* */
...
export function calculateKPIs(quote: Quote) {
  if (quote?.quoteKits?.[0]?.singleItems) {
    return calculateKPIsSingleItems(quote);
  } else {
    return calculateKPIsKit(quote);
  }
}

function calculateKPIsSingleItems(quote: Quote) {
  //alert(JSON.stringify({ priceKitsFinalInCents: quote.priceKitsFinalInCents }));
  let taxesInCents = getTaxesInCents(quote);

  let kitTaxesInCents = getKitTaxesInCents(quote);

  //kitTaxesInCents = taxesInCents;

  quote.kitRevenueInCents = getRevenueInCents(quote);
  if (quote.kitRevenueInCents) {
    quote.kitRevenueInCents = Math.round(
      quote.priceKitsFinalInCents +
        (quote.priceShippingInCents ?? 0) -
        kitTaxesInCents
    );
  } else {
    quote.kitRevenueInCents = Math.round(
      (quote.priceKitsFinalInCents + (quote.priceShippingInCents ?? 0)) *
        getTaxesFactor(quote)
    );
  }
  // alert(
  //   JSON.stringify({
  ...
  //     priceKitsFinalInCents: quote.priceKitsFinalInCents,
  ...
  // total
  if (taxesInCents) {
    quote.revenueInCents = Math.round(quote.priceInCents - taxesInCents);
  } else {
    quote.revenueInCents = Math.round(
      quote.priceInCents * getTaxesFactor(quote)
    );
  }
  quote.grossMarginInCents = Math.round(
    quote.revenueInCents -
      ...
      quote.paymentInterestAmountInCents
  );
...
function calculateKPIsKit(quote: Quote) {
  //Avaliar a mudança desse item para bater o cálculo
  quote.kitRevenueInCents = Math.round(
    (quote.priceKitsFinalInCents + (quote.priceShippingInCents ?? 0)) *
      getTaxesFactor(quote)
  );

  //Possível nova versão
  // quote.kitRevenueInCents = Math.round(
  //   quote.priceKitsFinalInCents * getTaxesFactor(quote) +
  //     (quote.priceShippingInCents ?? 0)
  // );
  ...
  // total
  quote.revenueInCents = Math.round(quote.priceInCents * getTaxesFactor(quote));
