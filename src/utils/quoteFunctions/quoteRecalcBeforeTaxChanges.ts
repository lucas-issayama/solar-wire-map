...
function quoteRecalc(quote: Quote, deleteApproval?: boolean) {
  ...
  //impostos
  if (isQuoteSingleItems(quote)) {
    quoteUpdated.priceInCents = Math.round(
      (quoteUpdated.priceInCents * taxesFactorKit) / taxesFactorSingleItems
    );
  }
...
export function calculateKPIs(quote: Quote) {
  ...
  console.log(`quote.priceKitsFinalInCents :${quote.priceKitsFinalInCents}`);

  console.log(` :${getTaxesFactor(quote)}`);
  console.log(
    ` Receita:${quote.priceKitsFinalInCents * getTaxesFactor(quote)}`
  );
  console.log(`assem:${quote.priceAssemblyInCents}`);
  quote.kitRevenueInCents = Math.round(
    (quote.priceKitsFinalInCents + (quote.priceShippingInCents ?? 0)) *
      getTaxesFactor(quote)
  );
  console.log(`  quote.kitRevenueInCents:${quote.kitRevenueInCents}`);
  // kit
