...
function updatePricesItemsAndTaxes(quote: Quote) {
  console.log(JSON.stringify({ priceInCents: quote.priceInCents }));
  let length = quote?.quoteKits?.[0]?.quoteKitItems?.length ?? 0;
  let total = 0;
  for (let i = 0; i < length; i++) {
    if (quote?.quoteKits?.[0]?.quoteKitItems?.[i]) {
      if (!quote?.quoteKits?.[0]?.quoteKitItems?.[i].deleted) {
...
function getPriceWithTaxesFactorSingleItems(quote: Quote) {
  let length = quote?.quoteKits?.[0]?.quoteKitItems?.length ?? 0;
  let total = 0;
  for (let i = 0; i < length; i++) {
    let quoteKitItem: any = quote?.quoteKits?.[0]?.quoteKitItems?.[i];
    ...
    if (quoteKitItem.type == "module") {
      ...
      itemPrice = (taxesFactorKit / taxesFactorSingleItems) * itemPriceRaw;
    }
...
// function getRevenueInCents(quote: Quote) {
//   //console.log("getRevenueInCents");
//   let priceRef =
//     quote.priceKitsFinalInCents + (quote.priceShippingInCents ?? 0);
//   console.log(JSON.stringify({ priceRef }));
//   console.log(JSON.stringify({ priceInCents: quote.priceInCents }));
//   let length = quote?.quoteKits?.[0]?.quoteKitItems?.length ?? 0;
//   let total = 0;
//   for (let i = 0; i < length; i++) {
...
//     if (quoteKitItem.type == "module") {
//       itemPrice = itemPriceRaw * taxesFactorKit;
//     } else {
//       itemPrice = itemPriceRaw * getTaxesFactor(quote);
//       // if (isQuoteSingleItems(quote))
//       //   itemPrice = itemPriceRaw * taxesFactorSingleItems;
//       // else itemPrice = itemPriceRaw * taxesFactorKit;
//     }
...
function quoteRecalc(quote: Quote, deleteApproval?: boolean) {
  ...
  //alert(quoteUpdated.priceKitsInCents);
  ...
  //quoteUpdated.priceInCents = Math.round(quoteUpdated.priceInCents / 0.7275);
...
export function calculateKPIs(quote: Quote) {
  ...
  // quote.revenueInCents = getRevenueInCents(quote);
