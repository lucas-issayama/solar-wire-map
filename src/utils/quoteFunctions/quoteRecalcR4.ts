...
function getTaxesFactor(quote: Quote) {
...
function quoteRecalc(quote: Quote, deleteApproval?: boolean) {
  ...
  if (quoteUpdated.paymentMethod?.id == 2) {
    ...
    quoteUpdated.paymentInterestAmountInCents = Math.round(
      paymentInfo?.totalInterestAmount / taxesFactorKit
    );
  ...
  console.log(JSON.stringify({ priceInCents: quoteUpdated.priceInCents }));
  ...
  console.log(
    JSON.stringify({
      priceInCents: quoteUpdated.priceInCents,
      kits: quoteUpdated.priceKitsFinalInCents,
    })
  );
  ...
  if (doRecalc) {
    ...
    console.log(JSON.stringify({ newPriceInCents }));
    if (quoteUpdated.paymentMethod?.id == 2) {
      let paymentInfo: any = getPaymentInfo(
        newPriceInCents,
        quoteUpdated.paymentNumberOfInstallments ?? 1
      ...
      //     quoteUpdated.paymentInterestAmountInCents / quote.priceInCents
      //   );
    }
    ...
    console.log(JSON.stringify({ newPriceKitsInCents }));
...
function getOptionalServicesFactors(quote: Quote) {
  ...
  //alert(JSON.stringify({ costsInPercentage }));
  ...
  //   //alert("itens avulsos");
  //   //alert(priceInCents);
  //   costsInPercentage.push(1 / 0.7275);
  //   priceInCents = Math.round(priceInCents / 0.7275);
  //   //alert(priceInCents);
  // }
...
function calculateKPIsSingleItems(quote: Quote) {
  console.log("calculateKPIsSingleItems");
  ...
  if (taxesInCents) {
    quote.revenueInCents = Math.round(
      quote.priceInCents - quote.paymentInterestAmountInCents - taxesInCents
    );
  } else {
    // quote.revenueInCents = Math.round(
    //   quote.kitRevenueInCents -
