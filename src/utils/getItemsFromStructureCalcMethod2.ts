...
export default function getItemsFromStructureCalcMethod2(
  _structure: Structure,
  _layoutItems: LayoutItem[],
  moduleProduct?: any
) {
  console.log("getItemsFromStructureCalcMethod2");
      ...
      for (let i = 0; i < supports?.length; i++) {
        ...
        if (structureItem?.product?.nModules) {
          if (i < supportsAvailableCount - 1) {
            ...
            console.log(
              `not last ${quantity},nModules:${structureItem?.product?.nModules}`
            );
          } else {
            //last
